...
import { createStartupIdea } from './ideaService';
import { analyzeStartupIdea } from './aiService';
 ...
 */
export async function createAndAnalyzeIdea(
    userId: string,
    ideaText: string,
    industry: string
): Promise<{
    ...
    try {
        ...
        console.log(`\n${'='.repeat(80)}`);
        console.log(`Starting idea creation and analysis pipeline for user: ${userId}`);
 ...
 */
export async function createAndAnalyzeIdeasBatch(
    userId: string,
...
> {
    try {
        ...
        const results = await Promise.all(
            ideas.map((idea) => createAndAnalyzeIdea(userId, idea.ideaText, idea.industry))
        );
